...
async function importToggl(authorization, dateToBook) {
    ...
    for (const workLogEntry of workLogEntries) {
        console.log(
            `Issue: ${workLogEntry.issueKey}, Duration: ${workLogEntry.durationMin}m, Description: ${workLogEntry.description}`,
        );
    }
    // check if workLogEntries contain undefined issueKeys
    if (_.some(workLogEntries, (w) => !w.issueKey)) {
        console.error(
